...
\begin{tikzpicture}[
  ...
  % Aktivitet (under sykkel, til venstre for gruppeaktivitet)
  \node[entity] (aktivitet) at (0, -5.5) {\entnavn{Aktivitet}\\
    \key{navn}\\
    kategori\\
    beskrivelse};
  ...
  % Idrettslagstime -- møter_til (0,n : 0,n) -- Profil
  \node[relationship] (moter-til-idrettslagstime) at (17, 2) {\normalsize møter\_til};

  % ========== SONE 4: Gruppeaktivitet (svak av Sal) og Profil ==========
  \node[relationship] (aktivitet-er-type) at (6, -5.5) {\normalsize er\_type};
  \node[weak entity] (gruppe) at (10, -5.5) {\entnavn{Gruppeaktivitet}\\
    \dashuline{ID}\\
    tidsrom\\
    \hspace{1em} start\\
    \hspace{1em} slutt\\
    \hspace{1em} dato};
  \node[relationship] (instruktor-for) at (14.5, -2.5) {\normalsize instruktør\_for};
  \node[relationship] (moter-til) at (14.5, -5.5) {\normalsize møter\_til};
  \node[attribute] (moter-til-tidspunkt) at (14.5, -7.2) {\itshape tidspunkt};
  \node[relationship] (pameldt-til) at (14.5, -8.5) {\normalsize påmeldt\_til};
  \node[attribute] (pameldt-til-pamelding-nummer) at (14.5, -10.2) {\itshape påmelding\_nummer};
  \node[entity] (bruker) at (21, -5.5) {\entnavn{Profil}\\
    \key{ID}\\
    type\\
    navn\\
    \hspace{1em} fornavn\\
  ...
  % ========== SONE 5: Profil → Prikk ==========
  \node[ident relationship] (bruker-prikk) at (22, -8.5) {\normalsize har\_prikk};
  \node[weak entity] (prikk) at (27, -8.5) {\entnavn{Prikk}\\
    \dashuline{ID}\\
    dato};
  ...
  % Aktivitet (1) -- er_type -- Gruppeaktivitet (n): enkel strek med pil mot Aktivitet, dobbel mot Gruppeaktivitet
  \draw[->] (aktivitet-er-type.west) -- (aktivitet.east);
  \draw[double, double distance=1.5pt] (aktivitet-er-type.east) -- (gruppe.west);
  ...
  \node[relationship] (er-medlem) at (23, 0.5) {\normalsize er\_medlem};
  \draw (bruker.north) -- (er-medlem.south);
  \draw (idrett.south) -- (er-medlem.north);

  % Idrettslagstime -- møter_til (0,n : 0,n) -- Profil
  \draw (idrettslagstime.south east) -- (moter-til-idrettslagstime.north west);
  \draw (moter-til-idrettslagstime.south east) -- (bruker.north west);
