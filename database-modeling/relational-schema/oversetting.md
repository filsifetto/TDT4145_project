...
har_fasilitet(senter_ID pk fk(Senter), fasilitet_ID pk fk(Fasilitet))
...
Gruppeaktivitet(senter_ID pk fk(Senter), sal_ID pk fk(Sal), ID pk, start, slutt, dato, aktivitet_navn fk (Aktivitet), instrukt_ID fk (Profil))
