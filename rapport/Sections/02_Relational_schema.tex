...
\rel{Tredem\o lle}{\pkfk{senter\_ID}, \pkfk{sal\_ID}, \pk{nummer}, produsent, maksimal\_hastighet, maksimal\_stigning}
\begin{fklist}
...
\paragraph{Sterke entiteter – Fasilitet, Idrettslag, Aktivitet.}
...
\paragraph{Senter.}
Primærnøkkelen er \textit{ID}. I tillegg har \textit{navn} en \texttt{UNIQUE}-restriksjon,
slik at \textit{navn} $\rightarrow$ \textit{ID, gate, nummer, fra, til} også er en
ikke-triviell FD. Venstresiden er en kandidatnøkkel og dermed en supernøkkel, så BCNF er oppfylt.
For \textit{(gate, nummer)} kunne man vurdert kandidatnøkkelstatus, men vi antar at
dette ikke holder: et senter kan i prinsippet flytte eller to sentre kan midlertidig befinne seg i
samme bygg. Adressen er dermed ikke garantert unik og regnes ikke som kandidatnøkkel.

\paragraph{Profil.}
Primærnøkkelen er \textit{ID}. I tillegg er både \textit{epost} og \textit{telefon}
unike (\texttt{UNIQUE}) for hver profil, slik at
\textit{epost} $\rightarrow$ \textit{ID, type, fornavn, etternavn, telefon}
og \textit{telefon} $\rightarrow$ \textit{ID, type, fornavn, etternavn, epost}
begge er ikke-trivielle FD-er. Venstresidene er kandidatnøkler og dermed supernøkler,
så BCNF er oppfylt.
